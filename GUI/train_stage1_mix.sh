...
torchrun --nproc_per_node="${GPUS_PER_NODE}" --nnodes="${NNODES}" --node_rank="${NODE_RANK}" --master_addr="${MASTER_ADDR}" --master_port="${MASTER_PORT}"  swift/cli/sft.py \
    ...
    --custom_dataset_info /mnt/dolphinfs/hdd_pool/docker/user/hadoop-basecv/hanwenkang/code/mllm/GUIRoboTron-Speech/GUI/data_unify/dataset_info_mix.json \
    --dataset ui_refexp omniact_fix ricoig16k guienv ricosca widget_captioning webui350k\
    en_aw en_uibert win macos macos macos amex2 \
    --deepspeed zero2
